...
double P1(int current)
{
    ...
    for(int i = 0; i <= 7; i++)
        for(int j = 0; j <= 7; j++)
            if(board[i][j] == 0)
            {
                if(queenmin[me][i][j] != inf)
                    p1 += power[7 - queenmin[me][i][j]];
                if(queenmin[3 - me][i][j] != inf)
                    p1 -= power[7 - queenmin[3 - me][i][j]];
            }
    p1 /= 64;
    return p1;
}

double P2(int current)
{
    DK(1);
    DK(2);
    double t = 0;
